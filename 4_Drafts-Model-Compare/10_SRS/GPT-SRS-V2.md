...
1. Introduction
   ...
   - **가정 및 의존성(Assumptions & Dependencies)**
     ...
     - 외부 LLM 공급자(예: GPT, Gemini)와 HWP/PDF 변환 솔루션은 상용 환경에서 SLA 수준의 안정성을 제공한다.
...
3. System Context and Interfaces
   ...
   - **외부 LLM API (예: GPT, Gemini)**: 사업계획서 서술 초안 및 보완 텍스트를 생성하기 위해 사용된다.
   - **결제 게이트웨이 (예: 카드 결제/간편결제)**: 구독 과금 및 결제 처리를 위한 외부 결제 시스템.
   - **문서 변환 서비스(HWP/PDF 변환기)**: 내부 또는 제3자 솔루션을 통해 제출용 HWP/PDF 파일을 생성한다.
   - **모니터링 및 로깅 플랫폼 (예: APM, 로그 수집기)**: 성능(p95), 오류율, 비용, 사용 행태를 모니터링한다.
   - **On-premise/Private Cloud 인프라**: 기업 고객이 자체적으로 운영하는 쿠버네티스/VM 환경.

   3.2 Client Applications

   - **웹 애플리케이션(Web Client)**: 크롬 등 현대 브라우저에서 접속 가능한 메인 사용자 인터페이스. Wizard, 편집기, 리포트 뷰어를 제공한다.
   - **관리자 콘솔(Admin Console)**: 템플릿/Validator 룰셋 관리, 모니터링 뷰, 운영 알림 확인용 웹 UI (내부 사용자용).

   3.3 API Overview

   - 시스템은 REST 기반의 백엔드 API를 제공하며, 웹 클라이언트는 HTTPS를 통해 JSON 형식으로 통신한다.
   - 주요 API 유형:
     - **프로젝트 관리**: `POST /projects`, `GET /projects/{id}`, `POST /projects/{id}/wizard/steps`
     - **문서 생성**: `POST /projects/{id}/documents/business-plan:generate`
     - **PMF 진단**: `POST /projects/{id}/pmf-report:generate`
     - **내보내기(Export)**: `GET /projects/{id}/export?format=hwp|pdf`
   - 모든 API 호출은 인증 토큰을 요구하며, 요청 크기·레이트 리밋·타임아웃은 NFR(REQ-NF-001 등)에 정의된 범위 내에서 동작해야 한다.
   ...
   sequenceDiagram
       ...
       Founder->>Web: 6. '초안 생성' 클릭
       Web->>API: 7. POST /projects/{id}/documents/business-plan:generate
       API->>LLM: 8. 섹션별 프롬프트 전송
       LLM-->>API: 9. 섹션별 서술 초안 반환
       API->>Store: 10. 초안 문서 및 메타데이터 저장
       API-->>Web: 11. 초안 문서 반환 (필수 항목 커버리지 포함)
       Web-->>Founder: 12. 제출 가능 수준 초안 표시
   ```
...
4. Specific Requirements
   ...
   | **REQ-FUNC-014** | 쉬운 모드/전문가 모드 전환 | 시스템은 동일 프로젝트에서 쉬운 모드(질문 기반)와 전문가 모드(문서 구조 기반) 간 손실 없이 전환을 지원해야 한다. | F4 | **Given** 사용자가 하나의 프로젝트에서 문서를 편집 중일 때 **When** '쉬운 모드/전문가 모드' 토글을 변경하면 **Then** 기존 입력 데이터가 손실 없이 각각 Wizard 뷰와 전체 문서 뷰로 재구성되어 표시되어야 한다. | Must |
   | **REQ-FUNC-015** | On-premise 배포 패키지 | 시스템은 On-premise/Private Cloud 환경을 위한 설치 패키지(예: Docker Compose)를 제공해야 한다. | F6 | **Given** 기업 고객이 On-premise 옵션을 계약한 상태에서 **When** 설치 패키지를 내려받아 안내서에 따라 설치하면 **Then** 외부 인터넷 연결 없이도 핵심 기능(F1~F5)을 사내 인프라에서 실행할 수 있어야 한다. | Should |
   | **REQ-FUNC-016** | 워크스페이스별 데이터 격리 설정 | 시스템은 조직 단위 워크스페이스마다 데이터 저장 위치 및 배포 옵션(Cloud/Private/On-premise)을 설정할 수 있어야 한다. | F6 | **Given** 조직 관리자 계정으로 워크스페이스를 생성하는 상황에서 **When** 배포 옵션을 선택하면 **Then** 해당 워크스페이스의 모든 데이터와 로그는 선택된 환경 내에만 저장·처리되어야 한다. | Should |
