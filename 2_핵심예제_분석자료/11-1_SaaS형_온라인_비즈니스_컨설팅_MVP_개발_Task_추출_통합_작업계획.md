...
## 단계 8. Task 정의 품질 점검 및 에이전트 파일럿 실행
...
- **에이전트 파일럿 실행**
  - Epic 0(프론트엔드 PoC) Task 묶음 1개 이상, REQ-FUNC 기반 기능 Task 묶음 1개 이상, REQ-NF 기반 Task 묶음 일부를 선정.
  - 실제 AI 에이전트에게 해당 Task 정의만을 입력으로 주고, 구현/테스트 결과를 평가하여 부족한 필드·가이드를 역으로 보완한다.
